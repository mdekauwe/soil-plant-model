...
This model couples stomatal conductance, plant hydraulics, ABA production, and
soil water balance to quantify (1) the hydraulic risk and (2) net cumulative
carbon assimilation of a plant over a designated dry period. It does not allow
for xylem refilling. For more details of model implementation, please refer to:

Feng et al. (2018), The ecohydrological context of drought and classification of
plant responses, Ecology Letters.

This code was written in Python 2.7 and requires the Python packages numpy,
scipy, matplotlib, and xlrd.
...
# low positive values used to constrain stomatal conductance gs
low_val_pos = 10**(-20)

# a multiplier to P50 beyond which xylem water potential is not allowed
pxf = 2.0

# minimum allowable leaf water potential
pl_min  = -50.0
...
class Canopy:
    def __init__(self, Gs_leaf, A_canopy, Amax, rho, lamABA, m):
        ...
        # max leaf conductance for H2O, mol/m2/s
        self.Gs_leaf = Gs_leaf

        # canopy area, m2
        self.A_canopy = A_canopy

        # maximum daily assimilation, mol/m2/d
        self.Amax = Amax

        # ratio of Rmax/Amax
        self.rho = rho

        # exponent on the stomatal response curve
        self.m = m

        # ABA production rates, mol ABA/m2-s-MPa
        self.lamABA = lamABA

    def g_canopy(self, gs):
        # Maximum canopy conductance per day, m3/d
        Sd, rhoH2O, Mw = (params_constants.Sd, params_constants.rhoH2O,
                          params_constants.Mw)
    ...
    def carboxyl_constant(self):
        ...
        # Amax and R converted to per second
        gmax, Amax, R = self.Gs_leaf, self.Amax/Sd, self.R()/Sd
...
class Stem:
    def __init__(self, L_stem, A_stem, Ksat_stem, P50_stem, a_stem=None,
                 c_stem=None, plc_form=None, stem_safety=None):

        # length of conducting stem, m
        self.L_stem = L_stem

        # sapwood area, m2
        self.A_stem = A_stem

        # maximum hydraulic conductivity, kg/m2-s-MPa
        self.Ksat_stem = Ksat_stem

        # water potential at 50% loss in maximum xylem conductivity, MPa
        self.P50_stem = P50_stem

        # functional form of xylem vulnerability, either 'exp' for exponential
        # or 'plc' for exponential sigmoidal
        self.plc_form = plc_form

        # parameter of xylem vulnerability in 'plc' form
        self.a_stem = a_stem

        # parameter of xylem vulnerability in 'exp' form
        self.c_stem = c_stem
    ...
    def k_stem(self, P_stem):
        ...
        kmax, a, c, P50 = (self.kmax_stem(), self.a_stem,
                           self.c_stem, self.P50_stem)
        if c:  # based on exponential function
            ...
            return kmax*(1.0-1.0  / \
                    (1.0+np.exp( a*( clamp(pxf*P50, P_stem,0) - P50) )))

    def k_stem_Kirchhoff(self, P):
        # Kirchoff transform - integrate vulnerability curve from -infinity
        # to P_stem (P)
        kmax, a, c, P50 = (self.kmax_stem(), self.a_stem,
                           self.c_stem, self.P50_stem)
        if c: # based on exponential function
            ...
            # constrain the domain of P to be negative
            k_stem = kmax*(np.log( \
                       np.exp(-a*P50) + np.exp(- a* clamp(pxf*P50,P,0)) )/a + P)
        return k_stem

    def Px_solvent(self, P_stem, percent):
        # helper function for solving for P_stem corresponding to designated
...
class Soil_root:
    def __init__(self, L_root, A_root, d_root, soil_type='loamy_sand'):
        # select soil types: "sand", "loamy_sand", "sandy_loam", "loam", "clay"
        # to set hydrological properties
    ...
    def kmax_root(self):
        ...
        nu, rhoH2O, g = (params_constants.nu, params_constants.rhoH2O,
                         params_constants.g)
        return nu*Ksat_soil/(rhoH2O*g) * \
                np.sqrt(self.A_root/(self.d_root*self.L_root))
    ...
    def k_soil_Kirchhoff(self, P):
        # Kirchhoff transform - integrated k_soil from -inf to P (which cannot
        # be greater than Psat_soil)
        b = soil_dict[self.soil_type]['b']
        Psat_soil = soil_dict[self.soil_type]['Ps']
        kmax = self.kmax_root()
        c = 2.0*b+3.0; d =4.0
...
class Soil:
    def __init__(self, soil_type):
        # select soil types: "sand", "loamy_sand", "sandy_loam", "loam",
        # "clay" to set hydrological properties
        self.soil_type = soil_type

        # Saturated soil hydraulic conductivity, m/d
        self.Ksat_soil = soil_dict[self.soil_type]['Ksat']

        # Soil wter potential near soil saturation, MPa
        self.Psat_soil = soil_dict[self.soil_type]['Ps']

        # Exponent on soil water retention curve
        ...
        # Hygroscopic point
        self.sh = soil_dict[self.soil_type]['sh']

        # Field capacity
        self.sfc = soil_dict[self.soil_type]['sfc']
...
class Whole_plant:
    def __init__(self, species):
        ...
        self.D0 = 0.0167             # Parameter combination, dimensionless,
                                     # see Dewar (2002)
    ...
    def stomata_func(self, ABA, VPD, P_leaf):
        ...
        rho, kcarbx, gmax = (self.canopy.rho, self.canopy.carboxyl_constant(),
                             self.canopy.Gs_leaf)

        # a1 is a constant used to designate gmax based on Amax, through
        # effects on the carboxylation capacity
        ...
        return a1*(1.0+rho)*kcarbx/(1.0+(VPD/(P_baro*D0))) * \
                np.exp( - beta*max(ABA,0))* np.exp(self.canopy.m * \
                max(P_leaf,pl_min))

    def log_stomata_func(self, ABA, VPD, P_leaf):
        # Logarithm of the previous stomatal conductance function
        # (Equation (5) in Feng (2018))
        rho, kcarbx, gmax = (self.canopy.rho, self.canopy.carboxyl_constant(),
                             self.canopy.Gs_leaf)
        a1 = 2.0*gmax/(1.6*kcarbx*(1.0+rho))
        D0, beta = self.D0, self.beta
        P_baro = params_constants.P_baro
        return np.log( a1*(1.0+rho)*kcarbx/(1.0+(VPD/(P_baro*D0))) ) - \
                max(ABA,0)*beta + self.canopy.m * max(P_leaf,pl_min)
    ...
    def ET_soil_func(self, P_soil, P_stem):
        ...
        return k_soilKirch( min(P_soil,0) ) - \
                    k_soilKirch( clamp(pxf*P50,P_stem,0) )
    ...
    def ET_stem_func(self, P_stem, P_leaf, Px_min):
        ...
        return k_stemKirch(Px_clamped) - k_stemKirch(min(P_leaf, P_stem)) + \
                max(0, k_stem(Px_min)*(P_stem - Px_min))
    ...
    def Pl_ET_func(self, ET, P_stem, Px_min):
        ...
        a, c, P50, kmax, k_stem = (self.stem.a_stem, self.stem.c_stem,
                                   self.stem.P50_stem, self.stem.kmax_stem(),
                                   self.stem.k_stem)

        # change the effective ET and P_stem processed over the untraversed
        # area of the vulnerability curve (v.c.) to account for no refilling

        # effective ET; if P_stem > Px_min, then account for area over kmax
        ET = ET - max(0, k_stem(Px_min)*(P_stem - Px_min))

        # effective P_stem; if P_stem > Px_min, then start traversing v.c.
        # from Px_min
        P_stem = min(P_stem, Px_min)

        # calculate leaf water potential from vulnerability curve ("exp" form)
        if c:
    ...
    def assimilation_daily(self,gs):
        # Returns daily assimilation rate based on carboxylation capacity,
        # mol/m2-d
        Sd, CO2conc, kcarbx = (params_constants.Sd, params_constants.CO2conc,
                               self.canopy.carboxyl_constant())
        R = self.canopy.R()/Sd
    ...
    def flux_solvent(self, (ET, P_stem, P_leaf, gs, ABA), P_soil, VPD, Px_min):
        ...
        return (ABA - self.ABA_func(P_leaf, ET),
                np.log( max(gs, low_val_pos) ) - \
                self.log_stomata_func(ABA, VPD, P_leaf),
    ...
    def flux_solver(self, P_soil, VPD, Px_min, init):
        # Solves coupled soil-plant hydraulic system - system of 3 equations
        # describing internal water states (ET, P_stem, P_leaf)
    ...
    def calculate_from_Px(self, Px, VPD, Px_min, Ps):
        # Calculates system variables based on stem water potential Px.
    ...
    def get_init_conditions(self, VPD, si, Px_min, Px_arr):
        # Outputs guesses for initial conditions used to solve the system,
        # based on inputs of soil moisture, VPD, and minimum stem water
        # potential Px_min Px_arr is the range of stem water potentials over
        # which potential initial conditions are tested.
        ...
        J_arr = np.zeros_like(Px_arr); ET_arr = np.zeros_like(Px_arr)
        ...
        # if minimum difference for water supply and demand is found close to
        # the edge of Px range
        except IndexError:
    ...
    def get_fluxes(self, VPD, s, Px_min, inits=None):
        # returns solutions to the soil-plant system (E, P_stem, P_leaf) along
        # with correponsding P_soil
    ...
    def get_fluxes_scalar(self, VPD, s, Px_min, Flux_inits=None):
        ...
        # Set a range of stem water potentials Px over which initial conditions
        # can be guessed
        Px_arr = self.soil.Psat_soil * \
                    np.linspace(s, self.soil.sh-0.001, 50)**(-self.soil.b)

        # flag will be turned to True only when solution is found with inputted
        # guesses
        sol_flag = False

        # if initial conditions have already been included in the arguments
        if Flux_inits is not None:
            sol0 = self.flux_solver_s(s, VPD, Px_min, Flux_inits)
            sol_flag = sol0.success
            if sol_flag:
                ET, P_stem, P_leaf, gs, ABA = sol0.x
        # if initial conditions have not been designated, or if previous
        # attempt at solving was not successful,
        # then we used guessed initial conditions to solve the system
        if (Flux_inits is None) or (sol_flag is False):
            Flux_inits_generated = self.get_init_conditions(VPD, s, Px_min,
                                                            Px_arr)
