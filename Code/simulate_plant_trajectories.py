...
from utility_functions import simulate_ps_t_nonlinearized
from params_soil import soil_dict
import matplotlib.pyplot as plt
import numpy as np


def plot_trajectories(plant, sfc, sst, n, Ks, tmax, VPD, s0=None, newfig=False,
                      lam=0.1, alpha=0.01, dt=0.1, n_trajectories=100):
    if newfig:
    ...
    ps, assm, _, pxmin, _, _, _ = simulate_ps_t_nonlinearized(n_trajectories,
                                                              tRun, dt, s0,
                                                              plant, VPD, lam,
                                                              alpha)
    ...
    print('HR: ', len(pxmin[pxmin<pxCrit]) / \
                        float(np.shape(pxmin)[0]*np.shape(pxmin)[1]))
...
if __name__ == '__main__':
    ...
    plot_trajectories(pine_plant, sfc, sst, n, Ks, tmax=90, VPD=2.0, s0=sst,
                      n_trajectories=20)
    plot_trajectories(juni_plant, sfc, sst, n, Ks, tmax=90, VPD=2.0, s0=sst,
                      n_trajectories=20)
    plt.show()
